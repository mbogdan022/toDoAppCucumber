...
function App() {
  ...
  const removeToDo = (id) => {
    ...
    const removeIndex = copyTodos.map(todo => todo.id).indexOf(id);
    ~removeIndex && copyTodos.splice(removeIndex, 1);
    setTodos(copyTodos)
  }
          ...
          {todos.map((todo) => (
            ...
            >
              ...
              <span className="flex-1"></span>
                ...
                <div onClick={() => removeToDo(todo.id)} className="ml-2" id="removeTodoBtn">
                  <XCircleIcon className="w-6 h-6" style={{ color: '#FF1010' }} />
                </div>
            </li>
          ))}
        </ul>
